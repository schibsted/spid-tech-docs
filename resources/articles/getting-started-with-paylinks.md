...
- You send the list of items to SchAcc, and get a unique URL back. Redirect the
  user to this Paylink URL.
- The user completes payment on the SchAcc website, and is redirected back to your site with an order ID.
...
- SchAcc will present the user with a summary of the items, and a total price.
...
To [create a Paylink](/endpoints/POST/paylink/), SchAcc expects a few parameters:
...
- `redirectUri` Where SchAcc should send the user after a successful payment
- `cancelUri` Where SchAcc should send the user after a cancelled payment
...
**NB!** If you store products in SchAcc, you can pass on `productId` when creating
...
Note that the SchAcc SDK will turn `items` into a JSON string for you.
...
interested in is the `shortUrl`. We'll use this to send the user to SchAcc for
payment.
...
SchAcc will redirect the user to your given `redirectUri` after a successful
...
The user might have logged in with a different user since we sent them to SchAcc
...
create a session for the user with information from SchAcc:
